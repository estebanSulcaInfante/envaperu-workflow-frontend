...
export const workspaceFeatures = [
  ...
  feature({ key: 'planning.demand', areaKey: 'planning', sectionKey: 'demand', label: 'Demanda, OP y plan', description: 'Demanda, cobertura, metas y OF/OA generadas.', path: '/planificacion', matches: ['/planificacion/:solicitudId'], requiredAny: ['OP_VER', 'OP_CREAR', 'OP_APROBAR', 'PLANIFICACION_CALCULAR'], icon: 'planning', defaultPriority: 10 }),
  ...
  feature({ key: 'production.fabrication', areaKey: 'production', sectionKey: 'fabrication', label: 'Fabricación · OF', description: 'Configura y libera órdenes de fabricación.', path: '/produccion/ordenes-fabricacion', requiredAny: ['OF_VER'], icon: 'production', defaultPriority: 20 }),
  feature({ key: 'production.machineWork', areaKey: 'production', sectionKey: 'machine-work', label: 'Jornadas y Trabajos de color', description: 'OT, trabajos, mangas, asignaciones y preetiquetas.', path: '/produccion/ots-mangas', requiredAny: ['OT_VER', 'PLAN_MANGA_VER'], icon: 'records', defaultPriority: 30 }),
  feature({ key: 'production.assembly', areaKey: 'production', sectionKey: 'assembly', label: 'Armado · OA', description: 'Libera y ejecuta órdenes de armado.', path: '/produccion/ordenes-armado', aliases: ['/produccion/ordenes-ensamble'], requiredAny: ['OA_VER'], icon: 'assembly', defaultPriority: 40 }),

  feature({ key: 'materials.preparation', areaKey: 'materials', sectionKey: 'preparation', label: 'Preparación de materiales', description: 'Requerimientos, reservas, emisiones y devoluciones.', path: '/materiales/preparaciones', matches: ['/materiales/preparaciones/:numeroOp'], aliases: ['/ordenes/:numeroOp/materiales', '/materiales'], requiredAny: ['OPERACION_PLANIFICAR', 'WIP_VER'], icon: 'reservations', defaultPriority: 20 }),
  feature({ key: 'materials.internalSupply', areaKey: 'materials', sectionKey: 'internal-supply', label: 'Abastecimiento interno', description: 'Picking, entrega y retorno hacia Armado.', path: '/produccion/abastecimiento', requiredAny: ['ABASTECIMIENTO_VER'], icon: 'internalSupply', defaultPriority: 30 }),
  feature({ key: 'materials.reprocessing', areaKey: 'materials', sectionKey: 'reprocessing', label: 'Merma, reproceso y molienda', description: 'Gestiona merma y recuperación operativa.', path: '/produccion/reproceso', requiredAny: ['MOLIENDA_VER'], icon: 'recycling', defaultPriority: 40 }),

  feature({ key: 'warehouse.receiving', areaKey: 'warehouse', sectionKey: 'receiving', label: 'Recepción y Calidad', description: 'Recibe mangas, ubica y resuelve Calidad.', path: '/produccion/recepcion-mangas', requiredAny: ['RECEPCION_MANGA_VER', 'CALIDAD_MANGA_VER'], icon: 'receipts', defaultPriority: 20 }),
  feature({ key: 'warehouse.kardex', areaKey: 'warehouse', sectionKey: 'inventory', label: 'Kardex y existencias', description: 'Consulta saldos y movimientos trazables.', path: '/produccion/kardex', requiredAny: ['INVENTARIO_VER'], icon: 'inventory', defaultPriority: 10, keywords: ['inventario', 'existencias', 'movimientos'] }),

  feature({ key: 'control.progress', areaKey: 'control', sectionKey: 'progress', label: 'Avance de planta', description: 'Seguimiento de cumplimiento y producción.', path: '/produccion/avance', aliases: ['/pesaje/avance'], requiredAny: ['WIP_VER'], icon: 'progress', defaultPriority: 10 }),
  feature({ key: 'control.alerts', areaKey: 'control', sectionKey: 'alerts', label: 'Alertas operativas', description: 'Excepciones que requieren atención.', path: '/produccion/alertas', requiredAny: ['ALERTA_VER'], icon: 'alerts', defaultPriority: 20 }),
  feature({ key: 'control.weighings', areaKey: 'control', sectionKey: 'weighings', label: 'Pesajes y correcciones', description: 'Histórico, conciliación y anulaciones.', path: '/produccion/pesajes', aliases: ['/pesaje/ordenes'], requiredAny: ['MANGA_PESAJE_VER'], icon: 'weighing', defaultPriority: 30 }),
  ...
  feature({ key: 'masters.colors', areaKey: 'masters', sectionKey: 'product-engineering', label: 'Colores y recetas', description: 'Color y formulación versionada.', path: '/datos-maestros/colores', requiredAny: ['ARTICULO_ADMINISTRAR', 'EMPAQUE_ADMINISTRAR'], icon: 'catalog' }),
