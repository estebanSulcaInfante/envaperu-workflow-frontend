...
export default function ProductionHistoryScm() {
  ...
  const [appliedFilters, setAppliedFilters] = useState(() => parseParams(searchParams));
  ...
  const load = async (nextFilters = filters) => {
    ...
    try {
      const payload = await listarProduccionHistoricaScm({ ...nextFilters, agrupaciones: nextFilters.agrupaciones.join(','), medidas: nextFilters.medidas.join(',') });
      if (sequence !== requestSequence.current) return;
      setItems(payload.items || []); setVisibility(payload.visibilidad || null); setAppliedFilters(nextFilters); setState('ready');
    } catch (cause) {
      if (sequence !== requestSequence.current) return;
      setError(cause?.response?.data?.error?.message || cause?.response?.data?.message || 'No se pudo cargar el histórico.'); setState('error');
    }
  };
  ...
  const update = (key) => (event) => {
    ...
    setFilters(next);
    if (key === 'agrupaciones' || key === 'medidas') load(next);
  };
  const exportFile = async () => {
    try { const response = await exportarProduccionHistoricaScm({ ...appliedFilters, agrupaciones: appliedFilters.agrupaciones.join(','), medidas: appliedFilters.medidas.join(',') }); const url = URL.createObjectURL(response.data); const anchor = document.createElement('a'); anchor.href = url; anchor.download = 'produccion-historica.xlsx'; anchor.click(); URL.revokeObjectURL(url); }
  ...
  const removeChip = (key, value) => {
    const next = { ...filters, [key]: Array.isArray(filters[key]) ? filters[key].filter((item) => item !== value) : '' };
    setFilters(next);
    load(next);
  };
  const selectedMeasures = useMemo(() => filters.medidas || [], [filters.medidas]);
  const appliedMeasures = appliedFilters.medidas || [];
  const appliedGroups = appliedFilters.agrupaciones || [];
  return <Box sx={{ p: { xs: 1.5, md: 3 }, maxWidth: 1500, mx: 'auto' }}>
    ...
    <Paper variant="outlined" sx={{ p: 1.5, mb: 2 }}><Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}><TextField size="small" type="date" label="Desde" InputLabelProps={{ shrink: true }} value={filters.desde} onChange={update('desde')} /><TextField size="small" type="date" label="Hasta" InputLabelProps={{ shrink: true }} value={filters.hasta} onChange={update('hasta')} /><TextField size="small" label="Buscar" value={filters.q} onChange={update('q')} /><FormControl size="small" sx={{ minWidth: 190 }}><InputLabel id="production-history-groups-label">Agrupar</InputLabel><Select multiple labelId="production-history-groups-label" label="Agrupar" value={filters.agrupaciones} onChange={update('agrupaciones')} renderValue={(selected) => selected.map((value) => groupLabels[value] || value).join(', ')}>{groups.map((group) => <MenuItem key={group} value={group}><Checkbox checked={filters.agrupaciones.includes(group)} /><ListItemText primary={groupLabels[group] || group} /></MenuItem>)}</Select></FormControl><FormControl size="small" sx={{ minWidth: 190 }}><InputLabel id="production-history-measures-label">Medidas</InputLabel><Select multiple labelId="production-history-measures-label" label="Medidas" value={selectedMeasures} onChange={update('medidas')} renderValue={(selected) => selected.join(', ')}>{measures.map((measure) => <MenuItem key={measure} value={measure}><Checkbox checked={selectedMeasures.includes(measure)} /><ListItemText primary={measure} /></MenuItem>)}</Select></FormControl></Stack><Stack direction="row" spacing={1} flexWrap="wrap" sx={{ mt: 1 }}>{filters.q && <Chip label={`Buscar: ${filters.q}`} onDelete={() => removeChip('q')} />}{filters.agrupaciones.map((value) => <Chip key={`g-${value}`} label={`Agrupar: ${groupLabels[value] || value}`} onDelete={() => removeChip('agrupaciones', value)} />)}{filters.medidas.map((value) => <Chip key={`m-${value}`} label={`Medida: ${value}`} onDelete={() => removeChip('medidas', value)} />)}</Stack></Paper>
