...
export default function ProductionOrderProgressScm() {
  ...
  const load = async () => {
    ...
    catch (cause) { setError(cause?.response?.data?.message || 'No se pudo cargar el avance de OF.'); setState('error'); }
  };
  ...
  return (
    <Box sx={{ p: { xs: 1.5, md: 3 }, maxWidth: 1500, mx: 'auto' }}>
      <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ sm: 'center' }} gap={1} sx={{ mb: 2 }}>
        ...
        <Button startIcon={<RefreshIcon />} variant="outlined" onClick={load} disabled={state === 'loading'}>Actualizar</Button>
      </Stack>
      <Paper variant="outlined" sx={{ p: 1.5, mb: 2 }}><TextField size="small" fullWidth label="Buscar OF, corrida, color u OT" value={query} onChange={(event) => setQuery(event.target.value)} /></Paper>
      {state === 'loading' && <Paper variant="outlined" sx={{ p: 4, textAlign: 'center' }}><CircularProgress aria-label="Cargando avance" /><Typography sx={{ mt: 1 }}>Cargando avance…</Typography></Paper>}
      {state === 'error' && <Alert severity="error" action={<Button color="inherit" onClick={load}>Reintentar</Button>}>{error}</Alert>}
      {state === 'ready' && !visible.length && <Paper variant="outlined" sx={{ p: 4, textAlign: 'center' }}><Typography>No hay objetivos de producción para mostrar.</Typography></Paper>}
